...
class ASVspoof5Dataset(Dataset):
    ...
    def __init__(
        ...
        print(f"  {len(self.samples):,} samples  ({n_bon:,} bonafide / {n_spo:,} spoof)")
...
class MP3AugmentedDataset(Dataset):
    ...
    transparently round-trips audio through an in-memory MP3 encode/decode
    at the given bitrate before returning it.

    Requires torchaudio with an ffmpeg or sox backend that supports MP3 write.
    """

    def __init__(self, dataset: Dataset, bitrate: str, sample_rate: int = 16_000):
        self.dataset     = dataset
        self.bitrate     = bitrate   # e.g. "128k", "256k"
...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device,
                    grad_accum_steps: int = 1, scaler=None):
    model.train()
    total_loss = 0.0
    n_batches  = 0
    correct    = 0
    ...
    for i, (audio, labels) in enumerate(dataloader):
        ...
        if (i + 1) % 50 == 0:
            print(f"    Batch {i+1:>5}/{len(dataloader)} │ "
...
def parse_args():
    ...
    p.add_argument("--train_meta", default="asvspoof5/ASVspoof5_protocols/ASVspoof5.train.tsv")
    p.add_argument("--dev_dir",    default="asvspoof5/flac_D")
    p.add_argument("--dev_meta",   default="asvspoof5/ASVspoof5_protocols/ASVspoof5.dev.track_1.tsv")
...
def main():
    ...
    # ── optimiser / loss ───────────────────────────────────────────────── #
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, eps=1e-7, weight_decay=0)
    ...
    print(f"  Batch / eff.    : {args.batch_size} × {args.grad_accum} grad-accum = {eff_batch}")
    print(f"  LR              : {args.lr}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        avg_loss, train_acc = train_one_epoch(
            ...
            grad_accum_steps=args.grad_accum, scaler=scaler,
        )

        dev_acc, dev_eer = evaluate(model, dev_loader, device, scaler=scaler)

        elapsed = time.time() - t0
        print(f"  loss {avg_loss:.4f} │ train acc {train_acc:.1f}% │ "
